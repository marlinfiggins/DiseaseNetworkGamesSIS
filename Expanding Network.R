...
MaxTime=500 ####Number of time steps
delta=0.2 #### Recovery
beta=0.3 ##### Beta
...
c2= 0.5 ##### Empathy
JerkFrac=0.50 ####Fraction who go out no matter what...
NewNodes=3
...
Growth=function(n, NewNodes, N, MaxTime, A, S, K, a){
  ...
  for (j in (n-NewNodes+1):n){ #####For Every New Node
    for (i in sample(1:(n-NewNodes))){  ###### Calculate Probility of Connection
      p[i]=K[i]/sum(K)
    }
...
plot(1:(MaxTime-1), InfecNum)
Individuals=numeric()
for (t in 1:(MaxTime-1)){
  Individuals[t]=length(S[which(!is.na(S[,t])), t])
}
plot(1:(MaxTime-1), InfecNum/Individuals)
